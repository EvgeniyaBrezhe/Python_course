...
# чтобы в итоге получился генератор нужного формата
# сет делает гарантированно случайные и разные числа
# переходы желательно не жестко вставлять
# я бы реализовывал вставку переноса отдельной функцией
# не начитается и не заканчивается переносом строки

# генерируем ключи, ключи жолжны быь уникальными, иаче они перезапишутся

# создать список списков - н штук объектов, обхекты - списочки, и каждый из них длины m